...
def rank_pocket_lining(residues, candidates, pockets, *, top_k=5, lining_mode="support"):
    ...
    membership must be unambiguous, which for a multi-chain target means the
    selected chains do not share numbers. Fail rather than silently
    ...
    if len(numbers) != len(ids):
        raise ValueError("pocket residue numbers must be unique across chains")
