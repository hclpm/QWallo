...
class _DomainSelect(Select):
    ...
    def accept_residue(self, residue):
        ...
        return flag == " " and self.start <= number <= self.end and is_aa(
...
def parse_fpocket_output(output_dir: Path, chain: str) -> tuple[Pocket, ...]:
...
def _domain_residues(structure, chain: str, start: int, end: int):
...
def _sample_enm_pockets(
    ...
    *,
    ...
    selected = _domain_residues(structure, chain, start, end)
    if not selected:
    ...
    with tempfile.TemporaryDirectory(prefix="qai-enm-pockets-") as temporary:
        ...
        for snapshot_index, displacement in enumerate(all_displacements):
            ...
            for residue, shift in zip(
                _domain_residues(deformed, chain, start, end),
                displacement,
            ):
                for atom in residue:
            ...
            writer.save(str(pdb_path), _DomainSelect(chain, start, end))
            pdb_paths.append(pdb_path)

        def detect(pdb_path):
            return (
                detector(pdb_path, chain)
                if detector is not None
                else _run_fpocket_pdb(pdb_path, chain, command)
            )
        if workers == 1:
